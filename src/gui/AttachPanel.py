...
class AttachPanel(wx.Panel):
	...
	def onBeginDrag(self, event): # wxGlade: AttachPanel.<event_handler>
		...
		print "dir (data): %s" % str (dir (data))
		print data.GetDataHere()
		print data.GetDataSize()
		print calcByteLen (os.path.join (self.currentPage.path, RootWikiPage.attachDir, self.getSelectedFiles()[0]))
		print "dir (dragSource): %s" % str (dir (dragSource))
