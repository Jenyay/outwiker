    ...
    def readOnlyWrap(*args, **kwargs):
        try:
            ...
            if ex.page is not None:
                logger.debug(
                    "ReadonlyException for page: %s (%s)",
                    ex.page.display_title,
                    ex.page.title,
                )
            else:
                logger.debug("ReadonlyException for unknown page: %s (%s)")

            showError(wx.GetApp().getMainWindow(), _("Page is opened as read-only"))
...
def findPage(application, page_id):
    ...
    if page_id.startswith(prefix):
        ...
        return application.wikiroot.getPageByUid(page_id)
...
def getAlternativeTitle(
    title: str,
    siblings: List[str],
    substitution: str = "_",
    template: str = "{title} ({number})",
) -> str:
    """
    ...
    result = newtitle
    n = 1
    while len(result.strip()) == 0 or result.lower() in siblings_lower:
