...
class NotesTreeRegistry(Registry):
    ...
    def get_page_registry(self, page):
        return self.get_section_or_create(
            REGISTRY_SECTION_PAGES, self.get_key_for_page(page)
        )
    ...
    def remove_page_section(self, page):
        ...
        if self.has_section(*path):
            ...
            logger.error("Item must be a section, not option: %s", page_key)
            self.remove_option(*path)

    def has_section_for_page(self, page):
        return self.has_section(REGISTRY_SECTION_PAGES, self.get_key_for_page(page))
