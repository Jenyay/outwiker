...
class WikiStyleOnlyAction(BaseAction):
    ...
    def run(self, params):
        ...
        styles_folder = (
            STYLES_BLOCK_FOLDER_NAME if is_block else STYLES_INLINE_FOLDER_NAME
        )
        ...
        option_name = (
            CONFIG_STYLES_BLOCK_OPTION if is_block else CONFIG_STYLES_INLINE_OPTION
        )
...
class WikiStyleAdvancedAction(BaseAction):
    ...
    @property
    def title(self):
        return _("Advanced style...")

    @property
    def description(self):
        return _("Show dialog for advanced text style settings")
    ...
    def run(self, params):
        ...
        styles_from_page = loadCustomStylesFromConfig(
            self._application.selectedPage.params, CONFIG_STYLES_SECTION, option_name
        )
        ...
        with StyleDialog(
            mainWindow, self._application, title, styles, example_html, tag
        ) as dlg:
