...
"""
        ...
        self._text_color_picker.Bind(EVT_COLOURSELECT, handler=self._onTextColorPicker)
        self._text_background_check.Bind(wx.EVT_CHECKBOX, handler=self._onUpdate)
        self._text_background_picker.Bind(
            EVT_COLOURSELECT, handler=self._onTextBackgroundPicker
        )
        self._custom_CSS_check.Bind(wx.EVT_CHECKBOX, handler=self._onUpdate)
        self._custom_CSS_editor.Bind(wx.stc.EVT_STC_CHANGE, handler=self._onEditCSS)
        self.updateExample()
    ...
    def getHTML(self):
        ...
        result += "{tag}.{class_name} {{\n".format(
            tag=self._tag, class_name=self.CSS_STYLE_NAME
        )
        ...
        if background_color:
            result += "background-color: {};\n".format(
                background_color.GetAsString(wx.C2S_HTML_SYNTAX)
            )
    ...
    def _create_gui(self):
        ...
        style_name_sizer.Add(
            style_name_label, flag=wx.ALIGN_CENTER_VERTICAL | wx.ALL, border=4
        )

        style_name_sizer.Add(
            self._style_name_combo,
        ...
        text_color_sizer.Add(
            self._text_color_check, flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, border=4
        )

        text_color_sizer.Add(
            self._text_color_picker,
        ...
        text_background_sizer.Add(
            self._text_background_check,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=4,
        )

        text_background_sizer.Add(
            self._text_background_picker,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_RIGHT | wx.ALL,
            border=4,
        )
        ...
        paramsSizer.Add(self._custom_CSS_editor, flag=wx.EXPAND | wx.LEFT, border=16)

        paramsPreviewSizer.Add(paramsSizer, flag=wx.EXPAND | wx.ALL, border=4)

        # HTML browser
        self._browser = getOS().getHtmlRender(self, self._application)
        paramsPreviewSizer.Add(self._browser, flag=wx.EXPAND | wx.ALL, border=4)

        # Ok / Cancel buttons
        okCancel = self.CreateButtonSizer(wx.OK | wx.CANCEL)
        paramsPreviewSizer.Add(
    ...
    def getTextColor(self) -> Optional[wx.Colour]:
        ...
        """
        return (
            self._text_color_picker.GetColour()
    ...
    def getBackgroundColor(self) -> Optional[wx.Colour]:
        ...
        """
        return (
            self._text_background_picker.GetColour()
    ...
    def setCustomBackgroundColors(
        self, colors: Iterable[Union[wx.Colour, str]]
    ) -> None:
    ...
    def getCustomStyleName(self) -> Optional[str]:
        ...
        """
        return (
            self._style_name_combo.GetStringSelection()
    ...
    def getCustomCSS(self) -> Optional[str]:
        ...
        """
        return (
            self._custom_CSS_editor.GetText().strip()
