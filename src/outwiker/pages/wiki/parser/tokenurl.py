...
class UrlToken (object):
    ...
    def getToken (self):
        token =  Regex ("((?# Начало разбора IP )(?<!\\.)([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])(?!\\.?[0-9])(?!\\w)(?# Конец разбора IP )|(((news|telnet|nttp|file|http|ftp|https)://)|(www|ftp)\\.)[-\\w0-9\\.]+[-\\w0-9]+)(:[0-9]*)?(/([-\\w0-9_,\\$\\.\\+\\!\\*\\(\\):@|&=\\?/~\\#\\%]*[-\\w0-9_\\$\\+\\!\\*\\(\\):@|&=\\?/~\\#\\%])?)?", re.IGNORECASE | re.UNICODE)("url")
