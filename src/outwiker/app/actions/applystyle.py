...
class SetStyleToBranchAction(BaseAction):
    ...
    def __applyStyle(self, application, page, style):
        if page.parent is not None and not page.readonly:
            ...
            application.onPageUpdateNeeded(page, PageUpdateNeededParams(False))
...
class AddStyleDialog(TestedDialog):
    ...
    def __createGui(self):
        self.SetTitle(_("Set style to branch"))

        styleLabel = wx.StaticText(parent=self, label=_("Style"))

        self.stylesCombo = wx.ComboBox(
            parent=self, style=wx.CB_DROPDOWN | wx.CB_READONLY
        )
        ...
        mainSizer.Add(styleLabel, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL, border=4)
    ...
    def __fillStyles(self, combobox):
        names_list = [_("Default")] + [
            os.path.basename(style) for style in self.__stylesList
        ]
