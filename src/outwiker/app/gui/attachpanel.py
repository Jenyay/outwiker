...
from outwiker.core.events import BeginAttachRenamingParams, AttachSelectionChangedParams
...
class AttachPanel(wx.Panel):
    ...
    def __init__(self, parent, application):
        ...
        self.__attachList = wx.ListCtrl(
            self, wx.ID_ANY, style=wx.LC_LIST | wx.LC_EDIT_LABELS | wx.SUNKEN_BORDER
        )
        ...
        self._bindAppEvents()
        addStatusBarItem(
            self._application.mainWindow, self._ATTACH_STATUS_ITEM, position=1
        )
    ...
    def _bindGuiEvents(self):
        self.Bind(wx.EVT_LIST_BEGIN_DRAG, self._onBeginDrag, self.__attachList)

        self.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self._onDoubleClick, self.__attachList)

        self.Bind(wx.EVT_CLOSE, self._onClose)

        self.Bind(
            wx.EVT_LIST_BEGIN_LABEL_EDIT, self._onBeginLabelEdit, self.__attachList
        )

        self.Bind(wx.EVT_LIST_END_LABEL_EDIT, self._onEndLabelEdit, self.__attachList)

        self.Bind(wx.EVT_LIST_ITEM_SELECTED, self._onAttachSelected, self.__attachList)

        self.Bind(
            wx.EVT_LIST_ITEM_DESELECTED, self._onAttachSelected, self.__attachList
        )

    def _unbindGuiEvents(self):
        self.Unbind(
            wx.EVT_LIST_BEGIN_DRAG, handler=self._onBeginDrag, source=self.__attachList
        )
    ...
    def updateAttachments(self):
        ...
        if page is not None:
            ...
            for fname in files:
                if (
                    showHiddenDirs
                    or not os.path.basename(fname).startswith("__")
                    or not page.isCurrentAttachSubdirRoot()
                    ...
                    self.__attachList.InsertItem(0, os.path.basename(fname), imageIndex)

            if not page.isCurrentAttachSubdirRoot():
                self.__attachList.InsertItem(
                    0, self.GO_TO_PARENT_ITEM_NAME, self.__fileIcons.GO_TO_PARENT_ICON
                )
    ...
    def _onDoubleClick(self, event):
        ...
        attach_dir = Attachment(self._application.selectedPage).getAttachPath(
            create=False
        )
        selected_item = event.GetText()
        subdir = os.path.normpath(os.path.join(page.currentAttachSubdir, selected_item))
        if self._isSubdirectory(attach_dir, subdir):
            ...
            actionController.getAction(AttachExecuteFilesAction.stringId).run(None)
    ...
    def SetFocus(self):
        ...
        if (
            self.__attachList.GetItemCount() != 0
            and self.__attachList.GetFocusedItem() == -1
        ):
    ...
    def selectAllAttachments(self):
        for index in range(self.__attachList.GetItemCount()):
            if self.__attachList.GetItemText(index) != self.GO_TO_PARENT_ITEM_NAME:
                self.__attachList.SetItemState(
    ...
    def _onEndLabelEdit(self, event):
        ...
        newName = os.path.join(
            self._application.selectedPage.currentAttachSubdir, event.GetLabel().strip()
        )

        logger.debug(
            "Renaming attachment: %s -> %s", self._oldEditedAttachName, newName
        )
...
class DropAttachFilesTarget(BaseDropFilesTarget):
    ...
    def OnDropFiles(self, x: int, y: int, files: List[str]) -> bool:
        ...
        if (
            self._application.wikiroot is not None
