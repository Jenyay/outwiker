...
from outwiker.core.spellchecker.spelldict import (
    get_words_from_dic_file,
    write_to_dic_file,
)
...
class SpellPanel(BasePrefPanel):
    ...
    def _createGui(self):
        ...
        self._skipDigitsCheckBox = self._createCheckBox(
            _("Skip words with digits"), mainSizer
        )
    ...
    def _createDictsList(self, mainSizer):
        ...
        mainSizer.Add(self.dictsList, 0, wx.ALL | wx.EXPAND, border=2)

    def _createCustomDict(self, mainSizer):
        dictLabel = wx.StaticText(self, label=_("Custom dictonary (one word per line)"))
        self.customDict = wx.TextCtrl(self, style=wx.TE_MULTILINE)

        mainSizer.Add(dictLabel, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, border=2)

        mainSizer.Add(self.customDict, 0, wx.ALL | wx.EXPAND, border=2)
    ...
    def Save(self):
        self._config.spellCheckerDicts.value = ", ".join(
            self.dictsList.GetCheckedStrings()
        )
