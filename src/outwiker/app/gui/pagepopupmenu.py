...
from outwiker.app.gui.pagedialog import createSiblingPage, createChildPage, editPage
...
class PagePopupMenu:
    ...
    def __bindPopupMenuEvents(self, popupMenu):
        ...
        popupMenu.Bind(
            wx.EVT_MENU, self.__onCopyAttachPath, id=self.ID_COPY_ATTACH_PATH
        )
        popupMenu.Bind(wx.EVT_MENU, self.__onCopyLink, id=self.ID_COPY_LINK)
        popupMenu.Bind(
            wx.EVT_MENU, self.__onOpenAttachFolder, id=self.ID_OPEN_ATTACH_FOLDER
        )
        popupMenu.Bind(
            wx.EVT_MENU, self.__onPropertiesPopup, id=self.ID_PROPERTIES_POPUP
        )
    ...
    def __createPopupMenu(self, popupPage):
        ...
        self.ID_ADD_CHILD = popupMenu.Append(
            wx.ID_ANY, actionController.getTitle(AddChildPageAction.stringId)
        ).GetId()
        self.ID_ADD_SIBLING = popupMenu.Append(
            wx.ID_ANY, actionController.getTitle(AddSiblingPageAction.stringId)
        ).GetId()
        self.ID_REMOVE = popupMenu.Append(
            wx.ID_ANY, actionController.getTitle(RemovePageAction.stringId)
        ).GetId()
        self.ID_RENAME = popupMenu.Append(
            wx.ID_ANY, actionController.getTitle(RenamePageAction.stringId)
        ).GetId()
        ...
        self.ID_PROPERTIES_POPUP = popupMenu.Append(
            wx.ID_ANY, actionController.getTitle(EditPagePropertiesAction.stringId)
        ).GetId()
    ...
    def __onCopyLink(self, _event):
        ...
        """
        assert self.popupPage is not None
        self._application.actionController.getAction(CopyPageLinkAction.stringId).run(
            page=self.popupPage
        )
    ...
    def __onCopyTitle(self, _event):
        ...
        """
        assert self.popupPage is not None
        self._application.actionController.getAction(CopyPageTitleAction.stringId).run(
            page=self.popupPage
        )
    ...
    def __onCopyPath(self, _event):
        ...
        """
        assert self.popupPage is not None
        self._application.actionController.getAction(CopyPagePathAction.stringId).run(
            page=self.popupPage
        )
    ...
    def __onCopyAttachPath(self, _event):
        ...
        """
        assert self.popupPage is not None
        self._application.actionController.getAction(CopyAttachPathAction.stringId).run(
            page=self.popupPage
        )
