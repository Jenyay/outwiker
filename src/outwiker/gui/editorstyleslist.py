...
class EditorStylesList(wx.Panel):
    ...
    def __createGui(self):
        ...
        self._fontBox = wx.StaticBox(self, label=_("Font"))
        self._colorPicker = wx.ColourPickerCtrl(self)
        self._bold = wx.CheckBox(self, label=_("Bold"))
        self._italic = wx.CheckBox(self, label=_("Italic"))
        self._underline = wx.CheckBox(self, label=_("Underline"))

    def __layout(self):
        fontBoxSizer = wx.StaticBoxSizer(self._fontBox, wx.VERTICAL)
        fontBoxSizer.Add(self._colorPicker, flag=wx.ALL | wx.EXPAND, border=2)
        fontBoxSizer.Add(self._bold, flag=wx.ALL, border=2)
        fontBoxSizer.Add(self._italic, flag=wx.ALL, border=2)
        fontBoxSizer.Add(self._underline, flag=wx.ALL, border=2)
        ...
        mainSizer.Add(fontBoxSizer, flag=wx.EXPAND | wx.ALL, border=2)
    ...
    def __bind(self):
        ...
        self._colorPicker.Bind(
            wx.EVT_COLOURPICKER_CHANGED, self._onStyleChanged, self._colorPicker
        )
    ...
    def _onStyleChanged(self, _event):
        ...
        if index >= 0:
            self._styles[index].fore = self._colorPicker.GetColour().GetAsString(
                wx.C2S_HTML_SYNTAX
            )
