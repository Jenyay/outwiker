...
class DateTimeFormatCtrl (wx.Panel):
    def __init__ (self, parent, initial=u""):
        """
        ...
        self._hints = [(u"%a", _(u"Abbreviated weekday name")),
    ...
    def _createGui (self):
        self.dateTimeFormatCtrl = FormatCtrl (self, self._initial, self._hints)
        ...
        dateTimeSizer.Add(self.dateTimeFormatCtrl, 0, wx.ALL | wx.EXPAND, border=2)
    ...
    def _updateExample (self):
        try:
            dateStr = unicode (datetime.now().strftime (self.GetValue()),
                               getOS().filesEncoding)
