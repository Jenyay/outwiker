...
class Shortcuter (object):
    ...
    def assignShortcuts(self):
        """
        Расставить клавишные сокращения (шорткаты) для доступа к меню
        (с использованием комбинаций, Alt+...)
    ...
    def _checkDuplicatesHotKeys(self, menu, duplicates, hotkeysDict={}):
        ...
        hotkeysDict - словарь полученных горячих клавиш.
            Ключ - горячая клавиша, значение - пункт меню
        """
    ...
    def _getExistingShortcuts(self, menu, duplicateFunc):
        ...
        duplicateFunc - функция, которая вызывается в случае обнаружения
            повторяющихся шорткатов. Функция принимает две строки с пунктами
            меню, где совпадают шорткаты.
        """
        # Ключ - буква, клавиатурного сокращения
        # (подчеркнутая буква, буква перед которой стоит &)
    ...
    def _assignMenuShortcuts(self, menu):
        ...
        def noneDuplicateFunc(shortcut, title):
            """
            Функция, используемая в случае, если при обнаружении повторения
            шортката делать ничего не надо
            """
            pass

        # Ключ - буква, клавиатурного сокращения (подчеркнутая буква,
        # буква перед которой стоит &)
        # Значение - название пункта меню.
    ...
    def _findNewShortcut(title, shortcuts):
        """
        Метод подбирает наиболее подходящее место для подчеркивания
        (если это возможно) и возвращает кортеж из нового заголовка меню и
        нового клавиатурного сокращения

        title - исходный заголовок меню
        shortcuts - словарь уже занятых шорткатов

        Если невозможно подобр шорткат, то возвращается кортеж из исходного
        заголовка и пустой строки
        """
        newtitle = title
        newshortcut = u""
    ...
    def _extractShortcut(title):
        """
        Возвращает букву, перед которой стоит знак &. Если такой буквы нет,
        возвращает пустую строку Учитывается тот факт, что строка && означает,
        что & надо просто показать
