...
class IconsCollectionTest(BaseWxTestCase):
    def setUp(self):
        super().setUp()
        self.temp_dir = mkdtemp(prefix="Абырвалг абыр")
        self.images_dir = "testdata/images"

    def tearDown(self):
        super().tearDown()
        removeDir(self.temp_dir)
    ...
    def testSingleRoot(self):
        collection = IconsCollection("testdata/icons/Без групп")
    ...
    def testGroups_01(self):
        collection = IconsCollection("testdata/icons/Иконки и группы")
        ...
        self.assertIsNone(collection.getCover(None))
        self.assertTrue(collection.getCover("Группа 1").endswith("__cover.png"))
        self.assertIsNone(collection.getCover("Группа 2"))

    def testGroups_02(self):
        collection = IconsCollection("testdata/icons/Только группы")
    ...
    def testAddGroup_01(self):
        collection = IconsCollection(self.temp_dir)
        self.assertEqual(collection.getGroups(), [])

        collection.addGroup("Новая группа")
        self.assertEqual(collection.getGroups(), ["Новая группа"])

        newcollection1 = IconsCollection(self.temp_dir)
    ...
    def testAddGroup_03(self):
        ...
        self.assertEqual(collection.getGroups(), ["Вторая группа", "Новая группа"])

        newcollection = IconsCollection(self.temp_dir)
        self.assertEqual(newcollection.getGroups(), ["Вторая группа", "Новая группа"])
    ...
    def testRenameGroup_01(self):
        ...
        newcollection1 = IconsCollection(self.temp_dir)
        self.assertEqual(newcollection1.getGroups(), ["Переименованная группа"])

    def testRenameGroup_02_invalid(self):
        collection = IconsCollection(self.temp_dir)
        collection.addGroup("Новая группа")

        self.assertRaises(ValueError, collection.renameGroup, "Новая группа", "")

        self.assertRaises(
            ValueError, collection.renameGroup, "Новая группа", "Абырвалг/Абырвалг"
        )
    ...
    def testRenameGroup_04_invalid(self):
        ...
        self.assertRaises(
            DuplicateGroupError, collection.renameGroup, "Новая группа", "Абырвалг"
        )
    ...
    def testRemoveGroup_03_invalid(self):
        ...
        self.assertRaises(KeyError, collection.removeGroup, "Абырвалг")

        self.assertRaises(KeyError, collection.removeGroup, "")
    ...
    def testAddIcons_04_invalid(self):
        ...
        self.assertRaises(KeyError, collection.addIcons, "Другая группа", fullPaths)

    def testAddIcons_png(self):
        files = ["new.png"]
        fullPaths = [os.path.join(self.images_dir, fname) for fname in files]
    ...
    def testAddIcons_19_resize(self):
        ...
        icons = {fname: os.path.join(self.temp_dir, fname) for fname in files}
    ...
    def testAddCover_01_group(self):
        ...
        coverpath = os.path.join(self.images_dir, "icon.png")

        newCoverPath = os.path.join(
            self.temp_dir, groupname, IconsCollection.COVER_FILE_NAME
        )
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(self.temp_dir, groupname, IconsCollection.COVER_FILE_NAME)
            )
        )
    ...
    def testAddCover_02_root(self):
        ...
        newCoverPath = os.path.join(self.temp_dir, IconsCollection.COVER_FILE_NAME)

        collection.setCover(None, coverpath)
        self.assertTrue(os.path.exists(newCoverPath))
        self.assertEqual(
            os.path.abspath(newCoverPath), os.path.abspath(collection.getCover(None))
        )

        collection.setCover(None, coverpath)
        self.assertTrue(
            os.path.exists(os.path.join(self.temp_dir, IconsCollection.COVER_FILE_NAME))
        )
    ...
    def testAddCover_04_root_invalid(self):
        ...
        coverpath = os.path.join(self.images_dir, "invalid.png")
    ...
    def testAddCover_05_root(self):
        ...
        coverpath = os.path.join(self.images_dir, "first.jpg")
        ...
        self.assertEqual(
            self.__getMaxImageSize(
                os.path.join(self.temp_dir, IconsCollection.COVER_FILE_NAME)
            ),
            16,
        )
    ...
    def testAddCover_06_group_svg(self):
        ...
        coverpath = os.path.join(self.images_dir, "image1.svg")

        newCoverPath = os.path.join(
            self.temp_dir, groupname, IconsCollection.COVER_FILE_NAME_SVG
        )
