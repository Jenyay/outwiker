...
class TextPageCreationTest(unittest.TestCase):
    ...
    def tearDown(self):
        self._application.wikiroot = None
        os.chmod(
            self._getConfigPath(self.wikiroot),
            stat.S_IRUSR | stat.S_IXUSR | stat.S_IWUSR,
        )
        os.chmod(
            self._getConfigPath(self.wikiroot["Страница 1"]),
            stat.S_IRUSR | stat.S_IXUSR | stat.S_IWUSR,
        )

        if self.wikiroot["Страница 2"] is not None:
            os.chmod(
                self._getConfigPath(self.wikiroot["Страница 2"]),
                stat.S_IRUSR | stat.S_IXUSR | stat.S_IWUSR,
            )
    ...
    def testDisplaySubpath(self):
        ...
        self.assertEqual(self.wikiroot["Страница 1"].display_subpath, "page 1")

        self.assertEqual(
            self.wikiroot["Страница 1/Страница 5"].display_subpath, "page 1/page 5"
        )

        self.assertEqual(
            self.wikiroot["Страница 2/Страница 3"].display_subpath, "Страница 2/page 3"
        )
    ...
    def testTypeCreation(self):
        textPage = TextPageFactory().create(self.wikiroot, "Текстовая страница", [])
        ...
        searchPage = SearchPageFactory().create(self.wikiroot, "Поисковая страница", [])
    ...
    def testIcon(self):
        ...
        self.assertEqual(
            os.path.basename(wiki["Страница 2/Страница 3/Страница 4"].icon),
            "__icon.gif",
        )
    ...
    def testReplaceIcon(self):
        ...
        self.assertEqual(os.path.basename(wiki["Страница 1"].icon), "__icon.ico")

        wiki["Страница 1"].icon = self.icons[1]
        self.assertEqual(os.path.basename(wiki["Страница 1"].icon), "__icon.png")

        wiki["Страница 1"].icon = self.icons[0]
        self.assertEqual(os.path.basename(wiki["Страница 1"].icon), "__icon.gif")

        wiki["Страница 1"].icon = self.icons[2]
        self.assertEqual(os.path.basename(wiki["Страница 1"].icon), "__icon.jpg")
    ...
    def testInvalidPageName(self):
        ...
        self.assertRaises(
            Exception, TextPageFactory.create, self.wikiroot, "+*5name:/\0", []
        )
    ...
    def testCreateTextContent(self):
        ...
        self.assertEqual(
            wiki["Страница 2/Страница 3/Страница 4"].content, "Тарам-пам-пам"
        )
