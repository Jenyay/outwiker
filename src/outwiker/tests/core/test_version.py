...
class VersionTest(unittest.TestCase):
    ...
    def testParse2(self):
        self.assertTrue(
            Version.parse("1.2.3") == Version(1, 2, 3), str(Version.parse("1.2.3"))
        )

    def testParse3(self):
        self.assertTrue(
            Version.parse("1.2.3 beta") == Version(1, 2, 3, status=StatusSet.BETA)
        )
        self.assertTrue(
            Version.parse(" 1.2.3 dev") == Version(1, 2, 3, status=StatusSet.DEV)
        )
        self.assertTrue(
            Version.parse("1.2.3  release ")
            == Version(1, 2, 3, status=StatusSet.RELEASE)
        )
        self.assertTrue(
            Version.parse("1.2.3") == Version(1, 2, 3, status=StatusSet.RELEASE)
        )
