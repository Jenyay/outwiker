...
def test_fromXmlChangeLog_versions_simple():
    ...
    assert changelog.versions[1].version == Version(2, 0, status=StatusSet.BETA)
...
def test_fromXmlChangeLog_versions_downloads_requirements_single():
    ...
    assert changelog.versions[0].downloads[0].requirements.os_list == ["Windows"]
...
def test_fromString_lang_ru():
    ...
    assert changeLog.versions[0].version == Version(1, 0, status=StatusSet.BETA)
    ...
    assert (
        changeLog.versions[0].downloads[0].href
        == "http://example.com/application_v1_01.zip"
    )
    assert (
        changeLog.versions[0].downloads[1].href
        == "http://example.com/application_v1_02.zip"
    )
    assert changeLog.versions[0].downloads[0].requirements.os_list == [
        "Windows",
        "Linux",
    ]
    assert changeLog.versions[0].downloads[1].requirements.os_list == ["Windows"]
    assert changeLog.versions[0].downloads[0].requirements.api_list == [
        (3, 666),
        (3, 777),
    ]
    assert changeLog.versions[0].downloads[1].requirements.api_list == [(3, 888)]

    assert changeLog.versions[1].version == Version(2, 0)
    assert changeLog.versions[1].changes[0].description == "Исправлена ошибка - 2"
    assert (
        changeLog.versions[1].changes[1].description == "Исправлена другая ошибка - 2"
    )
    assert (
        changeLog.versions[1].downloads[0].href
        == "http://example.com/application_v2_01.zip"
    )
    assert (
        changeLog.versions[1].downloads[1].href
        == "http://example.com/application_v2_02.zip"
    )
    assert changeLog.versions[1].downloads[0].requirements.os_list == [
        "Windows",
        "Linux",
    ]
    assert changeLog.versions[1].downloads[1].requirements.os_list == ["Windows"]
    assert changeLog.versions[1].downloads[0].requirements.api_list == [
        (3, 999),
        (3, 1111),
    ]
    assert changeLog.versions[1].downloads[1].requirements.api_list == [(3, 2222)]
