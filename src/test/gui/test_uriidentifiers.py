...
class UriIdentifierTest(TestCase, BaseOutWikerGUIMixin):
    ...
    def setUp(self):
        ...
        self.files = ["accept.png", "add.png", "anchor.png",
                      "файл с пробелами.tmp", "dir"]
...
class UriIdentifierIETest(UriIdentifierTest):
    ...
    def testFullPageLink2(self):
        """
        Тест на распознавание ссылок на страницы, когда движок IE считает,
        что это ссылка на файл
        """
        currentpage = self.wikiroot["Страница 1"]
        contentfile = self._getContentFile(currentpage)
    ...
    def testSubpath3(self):
        ...
        path = "".join([self._getContentFile(wikipage),
    ...
    def testLinkPage_03(self):
        ...
        currentpage = self.wikiroot["Страница 2/Страница 3"]
        contentfile = self._getContentFile(currentpage)
        link = generateLink(self.application, currentpage) + "/#anchor"
