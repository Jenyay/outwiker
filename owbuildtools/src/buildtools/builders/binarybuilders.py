...
class BasePyInstallerBuilder(BaseBinaryBuilder, metaclass=ABCMeta):
    ...
    def __init__(self, src_dir, dest_dir, temp_dir):
        ...
        # build/tmp/build_tmp
        self._workpath = os.path.join(temp_dir, "build_tmp")
    ...
    def get_params(self):
        params = [
            ...
            "--name outwiker",
            "--windowed",
            '--distpath "{}"'.format(self._dist_dir),
            '--workpath "{}"'.format(self._workpath),
            "--add-binary help" + os.pathsep + "help",
            "--add-binary iconset" + os.pathsep + "iconset",
            "--add-binary images" + os.pathsep + "images",
            "--add-binary locale" + os.pathsep + "locale",
            "--add-binary spell" + os.pathsep + "spell",
            "--add-binary styles" + os.pathsep + "styles",
            "--add-binary textstyles" + os.pathsep + "textstyles",
            "--add-binary plugins" + os.pathsep + "plugins",
        ]

        params += [
            "--hiddenimport {}".format(package) for package in self.get_includes()
        ]

        params += [
            "--exclude-module {}".format(package) for package in self.get_excludes()
        ]
    ...
    def build(self):
        ...
        print_info("Copy files to dest path.")
        shutil.copytree(os.path.join(self._dist_dir, "outwiker"), self._dest_dir)
...
class PyInstallerBuilderLinuxBase(BasePyInstallerBuilder):
    ...
    def _strip_binary(self):
        ...
        files_for_strip = list(strip_path.glob("libwx*.so.*")) + list(
            strip_path.glob("wx.*so")
        )

        for fname in files_for_strip:
            print_info("Strip {}".format(fname))
